...
export function createMachineService<T extends MachineSchema>(
  ...
  // the moment a machine that DOES call `refs.set`/`refs.get` (`@zag-js/collapsible`, added for
  // `ct-video-player`) actually ran - `refs.set is not a function`. Fixed generically using the
